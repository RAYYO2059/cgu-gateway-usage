...
def confidence_breakdown(result):
    ...
    **價目表補齊 12 個 model_family 之前**算的——當時有一批請求還落在
    ...
    重現座標是 tag，不是 commit
    -----------------------------
    本估算是「資料 × 外部價目 × 取價時間」的函數，不是只有資料的函數。
    要重現某個日期算出來的金額，需要**當時那一版** ref/pricing_table.csv。
    該檔隨 repo 版控，所以 tag 就是重現座標：

        docs-lite-2026-08   2026-08-27 取價的價目表，總額 $2,749.32

    上面留著的兩個舊值（51.8% / $1,424.65，以及更早的 50.2% / $1,369.89）
    **沒有對應的 tag，無法重現**。刻意不補：它們的用途是說明「按列歸屬 vs
    按價位歸屬」的差距有多大，兩個數字並列就講完了，不需要能重跑。
    要的是那個對比，不是那個金額。
