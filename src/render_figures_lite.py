...
def traffic_cost_mismatch(run_id: str, out_dir: Path) -> Path | None:
    """**用計數不用佔比，理由是計數不依賴抑制狀態。**

    服務憑證那一列單人佔 68.9% > 30%，登記為非自然人分組，所以
    ``request_share`` 與 ``cost_share`` 現在照常公布。但佔比能不能畫，取決於
    該列當下的抑制判定；它正是錯位最大的一列，判定一變，圖就少了最重要的點。

    改用 ``n_requests`` 與 ``cost_usd``——**計數不受抑制，計數是事實**。
    佔比只是計數除以固定總數，對數軸上兩者的形狀相同，換過去沒有資訊增益。
