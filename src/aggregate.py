...
# 明確登記為**非自然人**的分組值：{維度: {值, ...}}。
#
# 這份清單只豁免 dominant 那一條，不豁免 below_min_group_size。
#
# 為什麼只豁免 dominant
# --------------------
# dominant 要防的是「這一組的平均其實在描述一個人」。服務憑證不是人：
# 一個識別碼佔該組 68.9%，描述的是一支自動化流程，不是某位師生的行為。
# 對這種組套 dominant，擋不住任何再識別風險，卻讓同一個數字在
# 「表格顯示 —、csv 與散文卻寫出數字」之間自相矛盾——而被擋的比例
# 本來就能由已公布的計數直接算出來（60,075 / 120,520）。
#
# 為什麼保留 below_min_group_size
# ------------------------------
# 服務憑證背後可能是某個人自己架的服務。識別碼很少的時候，仍然有可能
# 從「這個服務的流量」反推到那個人。母數門檻是保守的那一道，留著。
#
# 為什麼是「維度 → 值的集合」而不是字串比對
# ----------------------------------------
# 必須同時比對維度與值。只比對值的話，任何維度底下只要出現同名的分組值
# 就會被放行；而「服務憑證」這四個字將來完全可能出現在別的維度裡。
# 沒登記的值一律照舊抑制——預設是保護不是放行，與 EXEMPT_DIMENSIONS 同理。
NON_PERSON_GROUPS: dict[str, frozenset[str]] = {
