...
## 本批基準值（2026-07-30 至 08-24）

上面三件必驗的是**步驟**，這裡是**基準**。下一批進來之後對照這張表：
數字本身不重要，**「偏離代表什麼」那一欄才是**。同一個偏離可能是上游變了、
使用行為變了、或我們的處理出錯了，三者的處置完全不同。

沒有哪一項有固定的容許範圍——這是第一批，樣本數是一。列在這裡是為了讓
下一批有東西可比，不是為了設閾值。
...
| 項目 | 本批 | 偏離代表什麼 |
| --- | --- | --- |
| 請求數 | 120,520 | 隨期間長度變，本身不是訊號。除以有資料的曆日數再比 |
| 識別碼數 | 357（275 人 + 82 服務憑證） | **暴增而人均暴跌 → 先查雜湊 salt 有沒有換**（見上面第 1 條）。單純成長是正常的 |
| 可歸屬比例 | 50.15% | 大幅下降代表服務憑證的自動化流程增加，不是人變少。它決定單位別分析的天花板 |
| 有資料曆日 | 24（期間 26 日，中斷 2 日） | 中間出現新的空白日要先判斷是中斷還是漏匯出，判準看邊界形狀（見坑 5） |

### 成本

| 項目 | 本批 | 偏離代表什麼 |
| --- | --- | --- |
| 總額 | $2,749.32（取價 2026-08-27） | **同一批資料重算得到不同金額是正常的**——價目變了。要比就固定價目表版本，見 tag `docs-lite-2026-08` |
| 四段佔比 | 未快取 30.73% ／快取讀 28.46% ／快取寫 18.76% ／輸出 22.05% | 快取讀佔比大跌 → 客戶端的請求模式變了（可能改用非 agent 型工具）。輸出佔比大漲 → 用途從「反覆問」轉向「產生長內容」 |
| 快取合計 | 47.22% | 同上。這個數字反映的是請求模式，不是單價 |
| price_confidence | confirmed 78.43%／inferred 13.55%／promotional 7.05%／provisional 0.97% | **promotional 佔比升高 → 有促銷價到期了還沒更新**。inferred 升高 → 有新模型只查得到現價 |
| unpriced_no_table 的 token | **0** | 大於 0 就是有新模型進來而價目表沒跟上。這一條是絕對值不是佔比，任何殘留都要查（見上面第 3 條） |
| priced 的 token 涵蓋率 | 97.77% | 下降代表本地模型或非 token 端點的用量上升，不一定是問題，但會讓總額的代表性下降 |

### 分類前置

| 項目 | 本批 | 偏離代表什麼 |
| --- | --- | --- |
| 母數鏈 | 33,666 → 31,011（20 字元下限）→ 17,365（四條規則） | 三個數字要一起看。中間那段掉太多代表短訊息變多 |
| 四條規則 | 向量化 587／影像 401／工具自動發出 2,387／流程中繼 10,271 | 工具自動發出大跌 → **客戶端改版，框架句變了，標記失效**。這是最容易靜默失效的一條 |
| 四項相加 + 剩餘 | 必須正好等於 31,011 | 對不上就是指派層級與去重層級不一致，程式會直接 raise |
| relay 兩條標記 | 100% 共現（各 10,507） | 不再相等 → 那支 ollama 分類器的外殼改版了，去看新版長什麼樣 |

### 結構

| 項目 | 本批 | 偏離代表什麼 |
| --- | --- | --- |
| 服務憑證佔請求 | 49.85% | 上升代表自動化流程增加。它直接壓低所有單位別分析的母體 |
| ollama 佔請求／佔 token | 34.90%／2.21% | **兩者差一個數量級是本批的特徵**：本地模型跑的是大量短請求。比例接近代表用途變了 |
| 最大遮罩帳號佔比 | 管理 78.69%／智慧運算 61.14%／教職員 45.01%／工 30.90%／醫 16.79% | 高的那幾個代表該單位的數字實質上在描述一個班（或一段號碼區間）。**大幅下降是好事**——代表使用者變分散了 |
| 學院數／系所身分數 | 4 ／ 25 個 dept_key | 出現新代碼時 `unmapped_dept_codes_lite` 會有值（見上面第 2 條） |

### 對照用的指令

```bash
python -m src.run metrics --line lite --publish   # 產出全部 csv
python -m src.classify_lite.prefilter             # 分類前置的四項計數
```
...
1. `prefilter` 印出的四項相加 + 剩餘母數 **必須正好等於相異內容總數**。
   對不上就是指派層級與去重層級不一致，見 ENGINEERING_NOTES〈指派層級與去重層級不一致〉。
